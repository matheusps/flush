...
const responsive = (
  styles: Exclude<FlushStyleObject, ThemeDerivedStyles>
) => (theme?: Theme) => {
  const next: Exclude<FlushStyleObject, ThemeDerivedStyles> = {}
  const breakpoints =
...
export const css = (args: FlushStyleObject = {}) => (
  ...
  for (const key in styles) {
    ...
    if (key === 'themeKey') {
      const themeKey = css(get(theme, val as string))(theme)
      result = { ...result, ...themeKey }
      continue
    }

    if (val && typeof val === 'object') {
      // TODO: val can also be an array here. Is this a bug? Can it be reproduced?
      result[key] = css(val as FlushCSSObject)(theme)
      continue
    }
