...
**<Code fontSize="150" marginRight="major-1">${type.name}</Code>** ${
        isShort
          ? `<Code fontSize="100" palette="primary">${type.encodedType}</Code>`
          : ''
      } ${
        type.isRequired
...
<details><Box use="summary" marginBottom="major-2"><strong>${
      typeMetaData.stateTypes.length
    } state props</strong></Box>
      ...
      uses.forEach((use) => {
        ...
        if (typeReference) {
          const useTypes = typeReference.types.filter(
            (type) =>
              !Boolean(types.find((refType) => refType.name === type.name))
          )
      ...
      if (symbol) {
        ...
        if (/^(?!(Local|use))[A-Z].*Props/.test(symbolName)) {
          ...
          const types = _.uniqBy(
            [
              ..._.get(typeReferences, `[Local${symbolName}].types`, []),
              ...extraTypes,
            ],
            'name'
          )
  ...
  docPaths.forEach((docPath) => {
    ...
    if (/#\s.*\s(API|Props|Return\sValues)/.test(mdContents)) {
      ...
      matches.forEach((match) => {
        ...
        let type
        if (
          match.includes('useState Return Values') ||
          match.includes('State Return Values')
        ) {
          const component = componentSection.split('.')[0]
          localType = `${component}StateReturn`
          type = 'state-return'
        } else if (
          match.includes('useState API') ||
          match.includes('State API')
        ) {
          const component = componentSection.split('.')[0]
